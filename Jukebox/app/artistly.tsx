...
export default function ArtistlyPage() {
  ...
  const newGame = async () => {
    ...
    try {
      ...
      allArtists = querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as Artist));
      const selectedArtist = allArtists[Math.floor(Math.random() * allArtists.length)];
  ...
  const onGuessSubmit = () => {
    const guessedArtist = allArtists.find(artist => artist.id === guess);

    if (guess === currentArtist?.id) {
      ...
      if (!guessedArtist || !currentArtist) return;

      setCorrectDebut(guessedArtist.Debut === currentArtist.Debut);
      setDebutLessThan(guessedArtist.Debut < currentArtist.Debut);

      setCorrectMembers(guessedArtist.Members === currentArtist.Members);
      setCorrectPopularity(guessedArtist.Popularity === currentArtist.Popularity);
      setPopularityLessThan(guessedArtist.Popularity < currentArtist.Popularity);

      setCorrectGender(guessedArtist.Gender === currentArtist.Gender);
      setCorrectGenre(guessedArtist.Genre === currentArtist.Genre);
      setCorrectNationality(guessedArtist.Nationality === currentArtist.Nationality);
  ...
  const getBoxStyle = (correct: boolean, revealed: boolean) => {
    if (correct) return styles.correct;
    if (revealed) return styles.incorrect;
    return null;
  };
  ...
  return (
    <>
      ...
      <View style={styles.container}>
        ...
        <Text style={styles.title}>{gameStatus === 'playing' ? '?' : currentArtist?.id}</Text>
        ...
        <View style={styles.artistInfoContainer}>
          ...
          <View style={styles.infoBoxes}>
            <View style={[styles.infoBox, getBoxStyle(correctDebut, triesLeft < 10)]}>
            ...
            <View style={[styles.infoBox, getBoxStyle(correctMembers, triesLeft < 10)]}>
            ...
            <View style={[styles.infoBox, getBoxStyle(correctPopularity, triesLeft < 10)]}>
            ...
            <View style={[styles.infoBox, getBoxStyle(correctGender, triesLeft < 10)]}>
            ...
            <View style={[styles.infoBox, getBoxStyle(correctGenre, triesLeft < 10)]}>
            ...
            <View style={[styles.infoBox, getBoxStyle(correctNationality, triesLeft < 10)]}>
